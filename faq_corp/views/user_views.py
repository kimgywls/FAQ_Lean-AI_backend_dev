...
class UserProfileView(APIView):
    ...
    def post(self, request):
        ...
        try:
            corp = Corp.objects.filter(corp_users=user).first()
        ...
        qr_code_path = os.path.join(settings.MEDIA_ROOT, f"qr_codes/corp_qr_{corp.corp_id}.png") if corp else None
        qr_code_url = (
            f"/media/qr_codes/corp_qr_{corp.corp_id}.png"
            if qr_code_path and os.path.exists(qr_code_path)
            else ""
        )
        logo_url = corp.logo.url if corp and corp.logo else ""
        ...
        response_data = {
            ...
            'business_name': corp.corp_name if corp else '',
            'business_address': corp.corp_address if corp else '',
    ...
    def put(self, request):
        ...
        if department_name:
            corp = user.corp  # 사용자와 연결된 Public 객체
            if corp:
