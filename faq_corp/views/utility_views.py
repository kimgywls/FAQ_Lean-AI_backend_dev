...
from ..serializers import (CorpRequestServiceSerializer)
...
class GenerateQrCodeView(APIView):
    ...
    def post(self, request):
        corp_id = request.data.get('store_id')  # 요청에서 store_id 받기
        #logger.debug(f"Request data store_id: {corp_id}")
        ...
        try:
            ...
            corp = Corp.objects.get(corp_id=corp_id, corp_users=request.user)
            #logger.debug(f"Store found for corp_id: {corp_id}, user: {request.user.username}")
        except Corp.DoesNotExist:
            #logger.debug(f"Store not found for corp_id: {corp_id}, user: {request.user.username}")
            return Response({'error': '스토어를 찾을 수 없습니다.'}, status=404)

        qr_url = f'https://mumulai.com/storeIntroduction/{corp.slug}'
        ...
        try:
            ...
            # QR 코드 이미지 저장 경로 설정
            qr_filename = f'corp_qr_{corp_id}.png'
            ...
            corp.qr_code = f'{settings.MEDIA_URL}qr_codes/{qr_filename}'
            corp.save()
            #logger.debug(f"Store updated with QR code path: {corp.qr_code}")

            return Response({
                'message': 'QR 코드가 성공적으로 생성되었습니다.',
                'qr_code_url': request.build_absolute_uri(corp.qr_code),  # 절대 URL 반환
...
class QrCodeImageView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # 사용자의 스토어 정보 가져오기
            corp = Corp.objects.get(corp_id=corp_id, corp_users=request.user)
            #logger.debug(f"Public object found: {corp}")

            if corp.qr_code:
                corp_name = corp.corp_name
                qr_code_path = corp.qr_code.lstrip('/')  # 경로에서 앞의 '/' 제거
                ...
                qr_content_url = f'https://mumulai.com/corpIntroduction/{corp.slug}'
            ...
            #logger.debug(f"Corp object not found for corp_id: {corp_id}, user: {request.user}")
            return Response({'error': 'corp not found'}, status=404)
        except Exception as e:
    ...
    def post(self, request):
        ...
        for file in files:
            try:
                ...
                # Store ID, 등록 시간 가져오기
                corp_id = request.user.corp.first().corp_id
    ...
    def post(self, request):
        ...
        if files:
            for file in files:
                ...
                req_ser_serializer = CorpRequestServiceSerializer(data=data)
