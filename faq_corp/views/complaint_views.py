...
import logging
from ..models import  Corp_User, Corp, Corp_Department, Corp_Complaint
from ..serializers import (CorpComplaintSerializer)
...
class ComplaintViewSet(ViewSet):
    ...
    def list(self, request):
        ...
        # 권한 확인
        self.authentication_classes = [CorpUserJWTAuthentication]
        ...
        corp = user.corp
        if not corp:
            return Response({"error": "해당 사용자는 기업이 없습니다."}, status=status.HTTP_404_NOT_FOUND)

        user_department = user.department
        if not user_department:
            return Response({"error": "사용자가 속한 부서가 없습니다."}, status=status.HTTP_404_NOT_FOUND)

        complaints = Corp_Complaint.objects.filter(
            corp=corp,
            department=user_department
        )
        serializer = CorpComplaintSerializer(complaints, many=True)
    ...
    def create(self, request):
        ...
        try:
            ...
            data['corp'] = corp.corp_id

            department_name = data.get('department')
            if department_name == '기타':
                department, _ = Corp_Department.objects.get_or_create(department_name='기타', corp=corp)
                data['department'] = department.department_id
            elif department_name:
                department = get_object_or_404(Corp_Department, department_name=department_name, corp=corp)
                data['department'] = department.department_id

            serializer = CorpComplaintSerializer(data=data)
            ...
            department_users = Corp_User.objects.filter(department=department)
    ...
    def customer_view(self, request):
        ...
        complaint = get_object_or_404(Corp_Complaint, complaint_number=complaint_number, phone=phone)
    ...
    @action(detail=True, methods=['patch'], authentication_classes=[CorpUserJWTAuthentication], permission_classes=[IsAuthenticated])
    def update_status(self, request, pk=None):
        """민원 상태 변경"""
        complaint = get_object_or_404(Corp_Complaint, pk=pk)
        new_status = request.data.get('status')

        if new_status not in dict(Corp_Complaint.STATUS_CHOICES):
    ...
    @action(detail=True, methods=['post'], authentication_classes=[CorpUserJWTAuthentication], permission_classes=[IsAuthenticated])
    def transfer(self, request, pk=None):
        ...
        new_department = get_object_or_404(Corp_Department, department_name=department_name, public=complaint.public)
        ...
        # 새 부서의 담당자들에게 SMS 전송
        department_users = Corp_User.objects.filter(department=new_department)
