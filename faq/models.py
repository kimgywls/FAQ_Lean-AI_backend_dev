...
class User(AbstractBaseUser):
    ...
    billing_key = models.OneToOneField(
        'BillingKey', on_delete=models.SET_NULL, null=True, blank=True, related_name='user_billing_key'
    )
...
class Subscription(models.Model):
    ...
    plan = models.CharField(max_length=20, choices=PLAN_CHOICES, default="BASIC") # 사용자의 구독 플랜을 관리
    is_active = models.BooleanField(default=True) # 구독 상태
    next_billing_date = models.DateField(default=date.today() + timedelta(days=30)) # 다음 결제 날짜

    # BillingKey 모델을 참조하여 결제 정보 관리
    billing_key = models.OneToOneField('BillingKey', on_delete=models.SET_NULL, null=True, blank=True, related_name='subscription_billing')
...
class BillingKey(models.Model):
    ...
    customer_uid = models.CharField(max_length=255, unique=True)  # PortOne에서 발급한 고유 사용자 결제 키
    imp_uid = models.CharField(max_length=255, blank=True, null=True) # 포트원 결제 API에서 사용되는 결제 고유 ID
