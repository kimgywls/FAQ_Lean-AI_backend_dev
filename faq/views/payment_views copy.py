...
'''
    ...
    @action(detail=False, methods=["POST"], url_path="process_kcp_subscription")
    def process_kcp_subscription(self, request):
        """
        KCP 정기결제 실행 API
        """
        user = request.user
        ...
        if not billing_key or not plan_id or not amount:
            return Response(
                {"success": False, "message": "billing_key, plan_id, amount가 필요합니다."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        url = "https://testspay.kcp.co.kr/kcppayment/"
        data = {
            "site_cd": settings.KCP_SITE_CD,
            "pay_method": "100000000000",
            "currency": "410",
            "approval_key": billing_key,
            "good_name": "정기 구독",
            "good_mny": amount,
            "buyr_mail": user.email,
            "ordr_idxx": "SUB-" + uuid.uuid4().hex[:20],
        }

        response = requests.post(url, data=data)
        response_data = response.json()

        if response_data.get("res_cd") == "0000":
            return Response(
                {"success": True, "message": "정기결제 성공", "data": response_data},
                status=status.HTTP_200_OK,
            )
        else:
            return Response(
                {"success": False, "message": response_data.get("res_msg", "정기결제 실패")},
                status=status.HTTP_400_BAD_REQUEST,
            )
